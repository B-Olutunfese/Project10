resource "aws_launch_configuration" "elb_launch_config" {
  ...
  security_groups = [aws_security_group.project10-sg.id]
...
resource "aws_autoscaling_group" "asg_project10" {
  ...
  desired_capacity = 4
  force_delete = true
  vpc_zone_identifier = [aws_subnet.project10-pub-sub1.id,aws_subnet.project10-pub-sub2.id,aws_subnet.project10-pub-sub3.id]
